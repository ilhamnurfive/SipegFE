...
export const actions = {
  getDetailKGB(state, payload) {
    return new Promise(res => {
      ...
      Listing.get_detail_usul_kgb(filter)
        .then((data) => {
          res({
            status: true,
            data: data.data
  ...
  postUsulKGB({
    ...
    return new Promise(res => {
      Posting.post_usul_kgb(payload)
        ...
        .catch((err) => {
          this._vm.$toast.error(err.response.data.errors.message)
          res({
            status: false
          })
        })
